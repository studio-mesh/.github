...
export class GitHubService {
    ...
    `;
		...
		try {
			...
			const orgResponse: ProjectResponse = await this.octokit.graphql(
				orgQuery,
				{
			...
			const userResponse: ProjectResponse = await this.octokit.graphql(
				userQuery,
				{
			...
			if (!userId) {
				throw new Error(
					"Project not found in either organization or user context",
				);
			}
			return userId;
		}
